...
function NewHome() {
  ...
  const handleButtonClick = (category, topicList) => {
    setTopics(topicList);
    setActiveButton(category);
  };

  const buttonStyle = (category) => ({
    backgroundColor: activeButton === category ? "#FF8C00" : "#FFF48F",
  ...
  return (
    <div style={{ width: "100%", minHeight: "100vh", backgroundColor: "#FAFAFA", display: "flex", flexDirection: "column", alignItems: "center", padding: "1rem" }}>
      <div style={{ display: "flex", justifyContent: "center", gap: "1rem", marginBottom: "1rem" }}>
        ...
        <button style={buttonStyle("Recent")} onClick={() => handleButtonClick("Recent", topicsRecent)}>Recent</button>
      </div>
